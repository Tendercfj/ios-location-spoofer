...
This tutorial teaches you how to use **Shadowrocket** to change your iPhone's location on supported older systems, **without jailbreaking, a computer, or a developer account**.
...
> **Version limit:** this method supports iOS 26.x and earlier, plus iOS 27 beta5 and earlier test builds. **iOS 27 beta6 and later, including iOS 27 stable, do not support the Shadowrocket MITM method.** Disable the module and use the computer-assisted CoreDevice/DVT fallback in [IOS27.md](IOS27.md).
...
3. Look at the **"domains"** list in the middle and confirm that it includes these five (they usually appear automatically after importing the module):
   ```
   gs-loc.apple.com
   gs-loc-cn.apple.com
...
> On iOS 26.x, only treat this as a cache issue after the Shadowrocket log contains `Location spoofer patched ... wifi/cell`. iOS 27 stable has no rewriteable response, so toggling Location Services or rebooting cannot fix it.
...
4. If the log confirms `patched` but the location has not changed, toggle Location Services again or reboot to clear the cache.
...
1. Change `debug=false` to `debug=true`, trigger a Maps location request, and search the Shadowrocket log for `Location spoofer`.
2. If the log contains `patched ... wifi/cell`, the rewrite succeeded; then handle the system cache as described in step 6.
3. If there is no script log and the Apple host shows TLS / `MITM failed` / certificate errors on iOS 27, this is the known pinning limit. Reinstalling the CA or rebooting will not fix it.
4. On iOS 26.x and earlier, check CA full trust, module status, HTTPS decryption, and all five exact hostnames.

**Q: What can I use on iOS 27 stable?**
Disable the Shadowrocket module and follow [IOS27.md](IOS27.md) to use the computer-assisted CoreDevice/DVT helper. There is currently no verified computer-free Shadowrocket bypass.

**Q:** Why didn't the domain name appear automatically after importing the module?
Manually add those five domains to the HTTPS decryption page, and remember to click the ✓ in the upper right corner to save.
...
```
Supported systems: import the module → enable HTTPS decryption and exact hosts → trust the CA → enable the proxy → save coordinates
→ trigger location and confirm `patched` in the log → handle cache if needed → verify in Maps

iOS 27 stable: disable the module → follow IOS27.md to prepare Developer Mode and a computer → use the DVT helper's set/clear commands
```
