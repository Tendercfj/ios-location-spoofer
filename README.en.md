...
> **iOS 27 stable is not supported by the MITM modules.** Starting with iOS 27 beta6, Apple location connections reject user CAs and no longer expose a `/clls/wloc` response that proxy scripts can rewrite. Hostname, certificate, regex, and JavaScript changes cannot bypass this. Use the computer-assisted CoreDevice/DVT fallback in [IOS27.md](IOS27.md).
...
The proxy modules support iOS 26.x and earlier, plus iOS 27 beta5 and earlier test builds. The iOS 27 beta6+ and stable limitation applies to every app below because all five rely on the same HTTPS MITM entry point.
...
On iOS 27 beta6 or later, `MITM failed` on an Apple location host is the expected result of certificate pinning. A fully trusted user CA still cannot satisfy the pinned Apple certificate/public key. Disable the module and use [the CoreDevice/DVT fallback](IOS27.md).

On iOS 26.x and earlier, this usually points to MITM hostname matching or iOS certificate full-trust state. Please verify:
...
4. If it still fails, try disabling QUIC/HTTP3-related options, reconnect the VPN, then toggle Location Services off/on. This does not bypass iOS 27 certificate pinning.
...
ios27-location-spoofer.sh           # Computer-assisted CoreDevice/DVT fallback for iOS 27
IOS27.md                            # iOS 27 status, setup, and commands
