...
用代理软件的 HTTPS 解密（MITM）功能，把 Apple 定位服务返回的坐标改掉，让 iPhone「以为」自己在世界任何角落。旧版系统上**无需越狱、无需电脑、无需开发者账号**。

> **iOS 27 正式版提示**：Apple 已阻止定位服务的用户 CA 中间人解密，Shadowrocket 拿不到可改写的 `/clls/wloc` 响应。修改模块域名、证书或 JavaScript 都无法绕过。需要改用电脑侧 CoreDevice/DVT 方案，见 [iOS 27 使用说明](IOS27.md)。
...
3. 导入对应模块并勾选启用
4. 断开重连 VPN，关开一次定位服务（iOS 26.x 的缓存场景可能需要重启）
5. 打开地图 App 验证

### 系统版本支持

| 系统版本 | Shadowrocket WLOC 模块 |
|---------|------------------------|
| iOS 26.x 及更早 | 支持；受网络定位与系统缓存影响 |
| iOS 27 beta1–beta5 | 旧方案可用，但稳定性有限 |
| iOS 27 beta6 及以后、iOS 27 正式版 | **不支持**；定位连接启用 certificate pinning，且不再产生可供脚本处理的 `/clls/wloc` 响应 |

这不是坐标解析或 Host 列表遗漏。项目曾尝试扩展 `gspe*` / `gsp*` 主机池，但这些主机同样在 TLS 握手阶段断开，扩展反而会导致系统无法定位，因此该改动已经回滚。Surge、Loon、Quantumult X 和 Stash 使用相同的 MITM 原理，也受同一限制。

如果使用 iOS 27 正式版，请关闭本模块，避免 Apple 定位连接持续失败，然后使用 [ios27-location-spoofer.sh](ios27-location-spoofer.sh) 的 CoreDevice/DVT 路径。它需要电脑、Developer Mode 和 USB 配对，不经过 Apple WLOC HTTPS 流量。

### iOS 26.x 缓存

只有当调试日志已经出现 `Location spoofer patched ... wifi/cell` 时，才说明脚本完成改写；此时位置不变才可能是 `locationd` 缓存。iOS 26.x 可按下面流程清缓存：
...
1. 把模块 `argument=` 里的 `debug=false` 改成 `debug=true`，触发一次地图定位，再到代理日志搜 `Location spoofer`
2. 日志出现 `patched ... wifi/cell`：拦截和改写成功；再检查目标坐标，并按上面的缓存流程操作
3. 日志没有脚本记录，且出现 Apple Host 的 TLS / `MITM failed` / 证书不可信：iOS 27 正式版属于已知 pinning，重装 CA、重启、加 Host 都无效，请关闭模块并改用 [iOS 27 方案](IOS27.md)
4. iOS 26.x 及更早系统没有脚本记录：再检查模块、HTTPS 解密、CA 完全信任和 5 个精确域名

### 看到 `MITM failed`？

在 iOS 27 beta6 及以后版本，这是 Apple 定位连接 certificate pinning 的预期表现，用户 CA 即使已“完全信任”也不能通过校验，`.sgmodule` 无法修复。请关闭模块并使用 [CoreDevice/DVT 替代方案](IOS27.md)。

在 iOS 26.x 及更早版本，通常是 MITM 主机名匹配或证书信任问题：
...
4. 仍不行可关闭 QUIC/HTTP3 后重连 VPN，再关开定位；这一步不能绕过 iOS 27 的 pinning
...
ios27-location-spoofer.sh            # iOS 27 电脑侧 CoreDevice/DVT 工具
IOS27.md                             # iOS 27 原因、准备与使用说明
